        ...
        queries.global_ctxt().unwrap().enter(|tcx| {
            ...
            for local_def_id in tcx.hir().body_owners() {
                ...
                if is_entry_point {
                    ...
                    // Replaces primary (autogenerated) entry point span (if any),
                    // with the first sub-diagnostic span (if any).
                    ...
                    for mut diagnostic in diagnostics {
                        ...
                        if is_in_entry_point {
                            let first_child_span = diagnostic
                                .children
                                .first()
                                .and_then(|child| child.span.primary_span());
                            if let Some(first_child_span) = first_child_span {
                                diagnostic.replace_span_with(first_child_span, true);
                                diagnostic.children.remove(0);
                            }
                        }
                        diagnostic.emit();
                    }
