...
impl<'compilation> rustc_driver::Callbacks for VerifierCallbacks<'compilation> {
    ...
    fn after_analysis<'tcx>(
        ...
        queries.global_ctxt().unwrap().enter(|tcx| {
            ...
            let crate_span = tcx.source_span(CRATE_DEF_ID);
            ...
            for local_def_id in tcx.hir().body_owners() {
                ...
                if let Some(entry_point_name) = entry_point_info {
                    ...
                    // and have no relation to the user-defined local item definitions.
                    ...
                    for mut diagnostic in diagnostics {
                        ...
                        if is_in_entry_point {
                            let first_local_span = diagnostic.children.iter().find_map(|child| {
                                child
                                    .span
                                    .primary_span()
                                    .filter(|span| crate_span.contains(span.source_callsite()))
                            });
                            let Some(first_local_span) = first_local_span else {
                                diagnostic.cancel();
                                continue;
                            };
                            ...
                            diagnostic.children.retain(|child| {
                                !child
                                    .span
                                    .primary_span()
                                    .is_some_and(|span| span.source_callsite() == first_local_span)
                            });
                        }
                        diagnostic.emit();
                    }
