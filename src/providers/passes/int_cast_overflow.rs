...
impl<'tcx, 'pass> LossyIntCastVisitor<'tcx, 'pass> {
    ...
    fn process_rvalue(&mut self, rvalue: &Rvalue<'tcx>, location: Location) {
        let Rvalue::Cast(CastKind::IntToInt, operand, ty) = rvalue else {
            return;
        };
        ...
        let max_scalar = match (ty.kind(), op_ty.kind()) {
            ...
            (
                ...
                TyKind::Uint(UintTy::U64 | UintTy::U128) | TyKind::Int(IntTy::I64 | IntTy::I128),
            ...
            (
                ...
                TyKind::Int(IntTy::I128) | TyKind::Uint(UintTy::U64 | UintTy::U128 | UintTy::Usize),
            ...
            (TyKind::Uint(UintTy::U64), TyKind::Uint(UintTy::U128) | TyKind::Int(IntTy::I128))
                if pointer_width != 64 =>
            {
